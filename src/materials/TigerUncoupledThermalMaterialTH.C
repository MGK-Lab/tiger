...
void
TigerUncoupledThermalMaterialTH::computeProperties()
{
...
Real
TigerUncoupledThermalMaterialTH::tau(Real & norm_v, Real & alpha, Real & diff, Real & dt, Real & h_ele, const RealVectorValue & v, const RealVectorValue & h) const
{
  Real tau = 0.0;
...
Real
TigerUncoupledThermalMaterialTH::Optimal(Real alpha) const
{
  Real s = 0.0;
  if (alpha < 0.01)
...
Real
TigerUncoupledThermalMaterialTH::Temporal(const RealVectorValue & v, const RealVectorValue & h, Real diff, Real dt) const
{
  Real s1, s2, s3;
  if (_effective_length<4)
  {
...
Real
TigerUncoupledThermalMaterialTH::DoublyAsymptotic(Real alpha) const
{
  Real s = 0.0;
  if (alpha <= 3.0)
    s = alpha / 3.0;
...
Real
TigerUncoupledThermalMaterialTH::Critical(Real alpha) const
{
  Real s = 0.0;
  if (alpha <= 1.0)
    s = 0.0;
...
RealVectorValue
TigerUncoupledThermalMaterialTH::EEL(const Elem * ele) const
{
  RealVectorValue L;
  L.zero();
  if (ele->dim() == 1)
...
void
TigerUncoupledThermalMaterialTH::ActualEEL(const Elem * ele, RealVectorValue & l, const unsigned & ind) const
{
  if (ind == 4)
